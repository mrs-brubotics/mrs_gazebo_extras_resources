...
public:
  ...
  /// \brief Internal representation of a joint
  struct fixedJoint
  {
    std::string       model1;
    physics::ModelPtr m1;
...
void GazeboRosLinkAttacher::Load(physics::WorldPtr _world, sdf::ElementPtr /*_sdf*/) {
  ...
  if (!ros::isInitialized()) {
    ROS_FATAL_STREAM("[GazeboRosLinkAttacher]: A ROS node for Gazebo has not been initialized, unable to load plugin. "
  ...
  ROS_INFO_STREAM("[GazeboRosLinkAttacher]: Attach service at: " << this->nh_.resolveName("attach"));
  this->detach_service_ = this->nh_.advertiseService("detach", &GazeboRosLinkAttacher::detach_callback, this);
  ROS_INFO_STREAM("[GazeboRosLinkAttacher]: Detach service at: " << this->nh_.resolveName("detach"));
  this->attach_typed_service_ = this->nh_.advertiseService("attach_typed", &GazeboRosLinkAttacher::attach_typed_callback, this);
  ROS_INFO_STREAM("[GazeboRosLinkAttacher]: AttachTyped service at: " << this->nh_.resolveName("attach_typed"));
  this->delete_service_ = this->nh_.advertiseService("delete", &GazeboRosLinkAttacher::delete_callback, this);
  ROS_INFO_STREAM("[GazeboRosLinkAttacher]: Delete service at: " << this->nh_.resolveName("delete"));
  this->apply_force_service_ = this->nh_.advertiseService("apply_force", &GazeboRosLinkAttacher::apply_force_callback, this);
  ROS_INFO_STREAM("[GazeboRosLinkAttacher]: ApplyForce service at: " << this->nh_.resolveName("apply_force"));
  ROS_INFO("[GazeboRosLinkAttacher]: Node initialized.");
  ...
  ROS_INFO_STREAM("[GazeboRosLinkAttacher]: ConnectWorldUpdateEnd");
}
...
bool GazeboRosLinkAttacher::do_attach(std::string model1, std::string link1, std::string model2, std::string link2, std::string joint_type) {
  ...
  if (this->getJoint(model1, link1, model2, link2, j)) {
    ROS_INFO_STREAM("[GazeboRosLinkAttacher]: Joint " << model1 << "->" << link1 << " to " << model2 << "->" << link2
                                                      << " already existed, reusing it for attach.");
    j.joint->Attach(j.l1, j.l2);
    ROS_INFO_STREAM("[GazeboRosLinkAttacher]: Attach finished.");
    return true;
  }
  j.model1 = model1;
  j.link1  = link1;
  ...
  if (b1 == NULL) {
    ROS_ERROR_STREAM("[GazeboRosLinkAttacher]: model " << model1 << " model was not found");
    return false;
  }

  physics::BasePtr b2 = this->world->BaseByName(model2);
  if (b2 == NULL) {
    ROS_ERROR_STREAM("[GazeboRosLinkAttacher]: model " << model2 << " model was not found");
    return false;
  }
  ...
  if (l1 == NULL) {
    ROS_ERROR_STREAM("[GazeboRosLinkAttacher]: link " << link1 << " was not found");
    return false;
  }
  j.l1 = l1;
  ...
  if (l2 == NULL) {
    ROS_ERROR_STREAM("[GazeboRosLinkAttacher]: link " << link2 << " was not found");
    return false;
  }
  j.l2 = l2;

  ROS_INFO_STREAM("[GazeboRosLinkAttacher]: Creating " << joint_type << " joint " << model1 << "->" << link1 << " to " << model2 << "->" << link2
                                                       << " and attaching");
...
bool GazeboRosLinkAttacher::do_apply_force(std::string model_name, std::string link_name, force_t force, torque_t torque) {
  physics::ModelPtr model = world->ModelByName(model_name);
  if (!model) {
    ROS_ERROR_THROTTLE(1.0, "[GazeboRosLinkAttacher]: Model '%s' not found", model_name.c_str());
    return false;
  }
  physics::LinkPtr link = model->GetLink(link_name);
  if (!link) {
    ROS_ERROR_THROTTLE(1.0, "[GazeboRosLinkAttacher]: Link '%s' not found in model", link_name.c_str());
    return false;
  }
...
bool GazeboRosLinkAttacher::attach_typed_callback(mrs_gazebo_extras_resources::GazeboAttachTyped::Request &req, mrs_gazebo_extras_resources::GazeboAttachTyped::Response &res) {
  ...
  if (!allowed_joint_type) {
    ROS_ERROR_STREAM("[GazeboRosLinkAttacher]: Unknown joint type " + req.joint_type);
    res.ok = false;
    return true;
  }
